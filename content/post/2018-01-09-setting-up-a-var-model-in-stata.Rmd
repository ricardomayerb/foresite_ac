...
The reduced-form VAR in (G2) is represented in the conventional autoregressive (AR) representation. In the AR representation, a set of N time-series are modeled as a function of their past values. However, equation (G2) cannot be used for structural economic inference due to two reasons. First, equation (G2) is written as a function of past values of the variables, but in order to analyze structural shocks, we must rewrite $Y_t$ as a function of current and past shocks of the variables. This can be achieved by writing the VAR in its moving average (MA) representation:
